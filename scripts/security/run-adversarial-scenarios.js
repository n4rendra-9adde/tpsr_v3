...
async function run() {
  const modelPath = path.join(__dirname, '../../docs/models/tpsr-adversarial-scenarios.v0.1.json');
  const mapPath = path.join(__dirname, '../../docs/models/tpsr-adversarial-control-map.v0.1.json');
  ...
  // We add live-confirmation.json later, but for manifest it might not be generated yet, or it's generated by another script?
  // Actually, we'll write an empty array or fake it for now, then the live script overwrites it.
  if (!fs.existsSync(path.join(outDir, 'live-confirmation.json'))) {
     fs.writeFileSync(path.join(outDir, 'live-confirmation.json'), JSON.stringify([]));
  }
  filesToHash.push('live-confirmation.json');
