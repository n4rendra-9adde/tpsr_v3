...
func (c *SBOMContract) SupersedeSBOM(ctx contractapi.TransactionContextInterface, sbomID string) error {
	if sbomID == "" {
	...
	if record.Status != StatusApproved && record.Status != StatusActive {
		return fmt.Errorf("can only supersede SBOMs in APPROVED or ACTIVE status, current status is %s", record.Status)
	}
